...
两个 app 都有顶层 `pipeline.py` / `server.py`，而 `sys.modules` 是全局的——谁先
被导入谁赢。写 tests/test_digest_cache.py 时就踩了一次：把 notes2insight 的目录塞进
`sys.path` 之后，summit2md 的 3 个测试导入到了错的 pipeline。当前用"按别名加载、
用完撤路径"绕开，真正的解法是在合并前端时把两个 app 变成正经的包。
