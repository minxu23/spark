...
**原先的前提又是错的。** 计划里写"议题小结和摘要卡是同一个抽象，统一成一套 schema"。
摊开一比根本不是：

| | summit2md 议题小结 | notes2insight 摘要卡 |
|---|---|---|
| 输出 | `TLDR: 一句话` + 3-8 条要点 | 8 个固定小节（主旨/论点/事实数据/时间锚点/主体/判断/分歧/原话）|
| 给谁看 | 人——嵌在演讲稿文档里读 | 机器——喂给跨笔记归纳 |
| 密度 | 低，中文散文 | 高，结构化抽取 |

强行合并只有两个结果：把演讲稿撑成结构化表格，或者把综合分析的输入抽稀。
**相似的是管道，不是提示词。**

实际做的：

- `core/digest.py`：缓存（内容寻址）+ 正文准备（去噪 / 截断并明确告知 / 按段落边界分块）
- notes2insight 转发到 core，键的算法一字未动（实测改动前后同一个键，没有再废一次缓存）
- **summit2md 补上它从来没有的 LLM 缓存**：原先只缓存字幕，所有模型调用——议题小结、
  发言人标注、演讲稿、翻译、大会总结、主题总结——每次都真打。现在全部走
  `_cached_summarize`，缓存落在 `<输出目录>/.cache/llm/`，和字幕缓存做邻居，
  跟着产物一起搬

summit2md 的键直接哈希提示词全文，notes2insight 的键哈希正文：因为前者的提示词里
没有会无谓变化的成分，后者的提示词带着笔记路径和本次选择里的序号。

受益场景：重试失败项、补生成演讲稿、换模型重跑总结、重新排序后重跑——之前这些都要
把已经算过的东西整份重算。

**提示词一个字没改**，所以产出质量不变，不需要新旧对照。
